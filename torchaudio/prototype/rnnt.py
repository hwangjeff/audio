...
class _Transcriber__HOLD(torch.nn.Module):
...
class _Transcriber(torch.nn.Module):
    ...
    def __init__(
        self, *, input_dim: int, output_dim: int, transformer: torch.nn.Module,
    ) -> None:
        super().__init__()
        """
        self.input_linear = torch.nn.Linear(
    ...
    def forward(
        ...
        """
        transformer_out, transformer_lengths = self.transformer(input, lengths)
        output_linear_out = self.output_linear(transformer_out)
        layer_norm_out = self.layer_norm(output_linear_out)
        return layer_norm_out, transformer_lengths
...
def conformer_rnnt_model(
    *,
    ...
    transcriber = _Transcriber(
        input_dim=256, output_dim=encoding_dim, transformer=conformer,
    )
    predictor = _Predictor(
        num_symbols,
        encoding_dim,
